...
/**
 * 敏感词配置和说明抽屉组件
 * 用于配置和说明新敏感词或编辑现有敏感词信息
 * 包含表单验证、JSON配置编辑器和Markdown编辑器等功能
 */
type BlackwordConfigDrawerProps = {
  /** 控制抽屉是否打开 */
  open: boolean
  /** 当抽屉打开状态改变时的回调函数 */
  onOpenChange: (open: boolean) => void
  /** 当前正在编辑的敏感词数据，如果为undefined则表示配置和说明新敏感词 */
  ...
  }: BlackwordConfigDrawerProps)
{
  ...
  // 初始化配置和说明敏感词的mutation
  const configBlackwordMutation = usePatchBlackwordMutation()

  // 初始化表单，设置验证规则和默认值
  const form = useForm<BlackwordConfigData>({
    resolver: zodResolver(BlackwordConfigSchema),
    ...
    >
      <SheetContent className='flex flex-col min-w-1/3'>
        <SheetHeader className='text-start'>
          ...
          <SheetDescription>
            配置和说明敏感词 (敏感词ID:{currentRow?.blackwords_id})
          </SheetDescription>
        </SheetHeader>
        {/* 将表单与react-hook-form实例连接 */}
        <Form {...form}>
          <form
            id='blackword-config-form'
        ...
        <SheetFooter className='gap-2'>
          ...
          <Button form='blackword-config-form' type='submit'>
            配置敏感词
          </Button>
        </SheetFooter>
      </SheetContent>
