 ...
 * - 批量操作表单数据验证结果
 */
export type BlackwordBatchSwitchData = z.infer<typeof BlackwordBatchSwitchSchema>
// endregion
...
export const BlackwordBatchExportSchema = z.object({
    ...
    blackwords_ids: z.array(
        z.number()
          .int()
          .positive('敏感词ID必须是正整数')
 ...
 * - 批量导出表单数据验证结果
 */
export type BlackwordBatchExportData = z.infer<typeof BlackwordBatchExportSchema>
// endregion
...
export const BlackwordCreateSchema = z.object({
    ...
    blackwords_slug: z.string()
      ...
      .min(2, '敏感词标识长度不小于2')
      .max(32, '敏感词标识长度不大于32')
      .regex(/^[a-zA-Z0-9\-_]{2,32}$/, '敏感词标识应该是字母、数字、连字符或下划线，长度在2到32之间'),
    // 敏感词集合
    blackwords_collection: z.array(z.string()),
    // 敏感词说明文档内容
    blackwords_readme: z.string(),
