...
interface IndustriesTableRowProps {
  row: Row<IndustryData>
  rowIdx: number
  isSelected: boolean
  getPinningStyles: (column: Column<IndustryData>) => PinningStyles
}

const IndustriesTableRowBase = ({
  row,
  rowIdx,
  isSelected,
  getPinningStyles,
}: IndustriesTableRowProps) => {
  return (
    <TableRow
      key={row.id}
      data-state={isSelected && 'selected'}
      className={`${rowIdx % 2 === 0 ? 'bg-background' : 'bg-muted/50'} hover:bg-muted ${isSelected ? '!bg-zinc-200 dark:!bg-slate-800' : ''}`}
    >
...
export const IndustriesTableRow = React.memo(IndustriesTableRowBase)
