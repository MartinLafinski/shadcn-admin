...
export function LinksTable({ data = [], pager = undefined, isLoading = false, isFetching = false }: DataTableProps) {
  ...
  const table = useReactTable({
    data, // 表格数据源
    columns: linksColumns, // 列定义，从外部导入
    // 启用核心行模型（基础渲染功能）
    ...
    globalFilterFn: (row, _, filterValue) => {
      const searchKeys = ['links_name', 'links_slug']
  ...
  return (
    ...
    >
      ...
      {/* 数据表格工具栏 */}
      {/* 包含搜索框和过滤器，允许用户搜索友链名称/标识，以及按启用状态过滤 */}
      <DataTableToolbar
        table={table} // 传递表格实例给工具栏组件
        // 不指定 searchKey 则使用全局过滤（支持跨多列 OR 搜索）
        // searchKey={["links_name", "links_slug"]}
        // 搜索框占位符文本
        searchPlaceholder='过滤 友链名称/标识...'
        // 定义过滤器配置
        filters={[
          {
            columnId: 'links_enabled', // 对应列的ID
            title: '可用', // 过滤器标题
      ...
      <LinkTableBulkActions table={table} />
    </div>
  )
}