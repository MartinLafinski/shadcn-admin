 ...
 * 行业管理页面路由配置
 *
 * 该路由对应路径为 /_authenticated/industries/，用于展示和管理用户的所有行业
 * 使用 TanStack Router 的路由定义方式创建页面路由
 *
 * 路由结构：
 * - 路径: /_authenticated/industries/
 * - 组件: RouteComponent (渲染 industries 组件)
...
export const Route = createFileRoute('/_authenticated/industries/')({
  ...
  validateSearch: (search) => {
    return z
      .object({
        industry_keyword: z.string().optional(),
 ...
 * 路由组件 - 行业管理页面
 *
 * 这是行业管理页面的入口组件，主要负责渲染 industries 功能组件
 * industries 组件包含了行业列表、创建、编辑等完整功能
 *
 * 开发指引：
