...
import { usePrejobQuery } from '../../api/prejobs.ts'

interface PrejobsInfoDialogProps {
  open: boolean
  onOpenChange: () => void
  prejobId: number
}

export function PrejobsInfoDialog({
  open,
  onOpenChange,
  prejobId,
}: PrejobsInfoDialogProps) {
  const { resolvedTheme } = useTheme()
  const { data: pj, isLoading, isError } = usePrejobQuery(prejobId)
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className='min-h-[30vh] max-w-3xl'>
          ...
          <div className='flex h-64 items-center justify-center gap-2'>
            ...
            <span className='text-muted-foreground'>加载预备作业数据...</span>
          </div>
        </DialogContent>
      </Dialog>
    )
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className='min-h-[30vh] max-w-md'>
          ...
          <div className='flex h-32 items-center justify-center'>
            <p className='text-muted-foreground'>无法加载预备作业数据</p>
          </div>
        </DialogContent>
      </Dialog>
    )
  ...
  const hasConfig = pj.prejob_config && Object.keys(pj.prejob_config).length > 0
  const totalSpider = pj.total_spider_task_count ?? 0
          ...
          <div className='flex items-start justify-between gap-4'>
            <div className='w-full min-w-0 space-y-1.5'>
              <DialogTitle className='flex items-center gap-3 text-2xl'>
                ...
                <span className='truncate'>{pj.prejob_name}</span>
                <Badge
                  variant={pj.prejob_enabled ? 'default' : 'secondary'}
                  className='text-xs'
                >
                  {pj.prejob_enabled ? '启用' : '停用'}
                </Badge>
              </DialogTitle>
              <div className='flex w-full gap-3 pt-2'>
                <div className='flex flex-1 items-center gap-2'>
                  ...
                  <div className='flex flex-col gap-0 text-xs text-muted-foreground'>
                    <span>{pj.prejob_slug}</span>
                    <span>ID: {pj.prejob_id}</span>
                  </div>
                </div>
                <div className='flex flex-1 items-center gap-2 border-l pl-4'>
                  <UserRoundPlusIcon className='h-3.5 w-3.5 shrink-0' />
                  <div className='flex flex-col gap-0 text-xs text-muted-foreground'>
                    <span>{pj.created_by || '-'}</span>
                    <span>{compactTime(pj.created_at)}</span>
                  </div>
                </div>
                <div className='flex flex-1 items-center gap-2 border-l pl-4'>
                  <UserRoundPenIcon className='h-3.5 w-3.5 shrink-0' />
                  <div className='flex flex-col gap-0 text-xs text-muted-foreground'>
                    <span>{pj.updated_by || '-'}</span>
                    <span>{compactTime(pj.updated_at)}</span>
                  </div>
                </div>
              </div>
            </div>
