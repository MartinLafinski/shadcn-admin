...
// 批量操作控件
import { EntrypointsTableBulkActions } from './actions/entrypoints-bulk-actions'
// 入口点表格数据列
import { entrypointsColumns } from './entrypoints-columns'
// 入口点数据同步
import {
  useEntrypointsSearch,
 ...
 * 入口点数据表格组件
 *
 * 此组件用于展示入口点列表数据，支持排序、过滤、搜索和分页功能
 * 主要功能包括：
 * - 显示入口点基本信息（名称、标识、URL等）
 * - 支持按入口点名称/标识/URL搜索
 * - 支持按启用状态过滤
 * - 支持列排序和分页
 * - 响应式设计，适配移动端
...
interface DataTableProps {
  /**
   * 入口点数据列表
   * 类型为 EntrypointItemData 数组，包含入口点的基本信息
   */
  data: EntrypointItemData[] | undefined
  /**
   * 分页信息
 ...
 */
const EntrypointsTableBase = ({
  data = [],
  pager = undefined,
  isLoading = false,
  isFetching = false,
}: DataTableProps) => {
  // 从 context 获取搜索参数和操作方法
  const { searchParams } = useEntrypointsSearch()
  const { setSearchParams } = useEntrypointsActions()
  // 获取导航信息
  ...
  // 列固定状态
  const pinnedLeftIds = useMemo(
    () => [
      'select',
      'entrypoint_id',
      'entrypoint_name',
      'entrypoint_status_mini',
    ],
    []
  )
  ...
  const table = useReactTable({
    data, // 表格数据源
    columns: entrypointsColumns, // 列定义，从外部导入
    // 启用核心行模型（基础渲染功能）
  ...
  return (
    ...
    >
      ...
      {/* 数据表格工具栏 */}
      {/* 包含搜索框和过滤器，允许用户搜索入口点名称/标识/URL，以及按启用状态过滤 */}
      <DataTableToolbar
        table={table} // 传递表格实例给工具栏组件
        // 不指定 searchKey 则使用全局过滤（支持跨多列 OR 搜索）
        // searchKey={["entrypoint_name", "entrypoint_slug", "entrypoint_url"]}
        // 搜索框占位符文本
        searchPlaceholder='过滤 入口点名称/标识/URL...'
        // 定义过滤器配置
        filters={[
          {
            columnId: 'entrypoint_enabled', // 对应列的ID
            title: '可用', // 过滤器标题
            options: enableLabels, // 过滤选项，从外部导入的启用状态标签
          },
          {
            columnId: 'entrypoint_locked',
            title: '锁定',
            options: lockedLabels,
          },
          {
            columnId: 'entrypoint_paused',
            title: '运转',
          ...
          <Table>
            ...
            <TableBody>
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell
                    colSpan={entrypointsColumns.length}
                ...
                <TableRow>
                  ...
                  <TableCell
                    colSpan={entrypointsColumns.length} // 横跨列数等于列定义的长度
      ...
      <EntrypointsTableBulkActions table={table} />
    </div>
  )
}
export const EntrypointsTable = React.memo(EntrypointsTableBase)
