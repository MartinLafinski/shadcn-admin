...
export function ReqsProvider({
    ...
    // 管理搜索参数状态
    const [searchParams, setSearchParams] = useState<ReqSearchParams>(initialSearchParams)
    ...
    }, [search.task_id, search.result_type, search.result_category, search.page, search.size])

    return (
        <ReqsContext value={{ open, setOpen, currentRow, setCurrentRow, currentJob, setCurrentJob, searchParams, setSearchParams }}>
            {children}
        </ReqsContext>
    )
}
