...
interface PrejobsTableRowProps {
  row: Row<PrejobItemData>
  rowIdx: number
  isSelected: boolean
  getPinningStyles: (column: Column<PrejobItemData>) => PinningStyles
}

function PrejobsTableRowBase({
  row,
  rowIdx,
  isSelected,
  getPinningStyles,
}: PrejobsTableRowProps) {
  return (
    <TableRow
      key={row.id}
...
export const PrejobsTableRow = PrejobsTableRowBase
