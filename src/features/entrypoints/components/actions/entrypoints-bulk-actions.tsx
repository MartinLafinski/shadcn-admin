...
// 批量更新API调用
import { useBatchSwitchEntrypointsMutation, useBatchExportEntrypointsMutation } from '@/features/entrypoints/api/entrypoints'
// 可用性标签
...
export function EntrypointsTableBulkActions<TData>({
  ...
  const handleBulkStatusChange = async (status: boolean) => {
    // 从表格选中行中提取入口点数据
    const selectedEntrypoints = selectedRows.map((row) => row.original as EntrypointItemData)
    // 提取入口点ID数组，用于API调用
    const selectedEntrypointIds = selectedEntrypoints.map((entrypoint) => entrypoint.entrypoint_id)
    // 显示操作进度和结果提示
    toast.promise(
