...
// 批量导出API调用
import { useBatchExportTasksMutation } from '@/features/jobs/api/jobs'
// 数据结构
 ...
 */
type JobsTableBulkActionsProps<TData> = {
  table: Table<TData>
}
 ...
 * @param {JobsTableBulkActionsProps<TData>} props - 组件属性
 ...
 * 2. 支持批量导出任务
 */
export function JobsTableBulkActions<TData>({
  table,
}: JobsTableBulkActionsProps<TData>) {
  // 获取当前选中的行数据
  ...
  const handleBulkExport = async () => {
    // 从表格选中行中提取任务数据
    const selectedJobs = selectedRows.map((row) => row.original as JobItemData)
    // 提取任务ID数组，用于API调用
    const selectedJobIds = selectedJobs.map((job) => job.task_id)
    // 导出成功后的处理
    toast.promise(
      // 发起批量导出API请求
      exportMutation
