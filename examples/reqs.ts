 ...
 * 导出网站请求结果
 *
 * 此函数用于向后端API发送请求，导出指定网站的所有请求结果
 *
 * @param website_id - 需要导出请求结果的网站的唯一标识符（ID）
 ...
 * // 导出ID为1的网站的请求结果
 * try {
 *   const response = await exportWebsiteReqs(1)
 *   console.log("网站请求结果导出成功")
 * } catch (error) {
 *   console.error("网站请求结果导出失败:", error)
 * }
 *
 * 注意事项:
 * - 该函数会向 /reqs/websites/{website_id}/ 端点发送PUT请求
 * - 导出操作会生成导出文件，包含该网站的所有请求结果
 * - 函数内部使用handleResponse进行错误处理，确保错误被正确抛出
 * - 成功导出后，后端通常返回200状态码和空对象
 * - reqs API 可能不需要认证，但为了兼容性，仍然集成了 Clerk 认证
 */
export const exportWebsiteReqs = async (
  website_id: number,
  token: string | null = null
): Promise<Response> => {
  const headers: Record<string, string> = {
  ...
  const response = await fetch(`${API_BASE_URL}/reqs/websites/${website_id}/`, {
    method: 'PUT',
    headers,
  })
  return await handleResponse(response)
 ...
 * 清空网站请求结果
 *
 * 此函数用于向后端API发送DELETE请求，清空指定网站的所有请求结果
 *
 * @param website_id - 需要清空请求结果的网站的唯一标识符（ID）
 *                   必须是有效的数字ID，对应数据库中存在的网站记录
 *
 * @param token - 鉴权token（可选，reqs API 可能不需要认证）
 *
 ...
 * // 清空ID为1的网站的请求结果
 * try {
 *   const response = await clearWebsiteReqs(1)
 *   console.log("网站请求结果清空成功")
 * } catch (error) {
 *   console.error("网站请求结果清空失败:", error)
 * }
 *
 * 注意事项:
 * - 该函数会向 /reqs/websites/{website_id}/ 端点发送DELETE请求
 * - 清空操作会永久删除指定网站的所有请求结果，请谨慎操作
 * - 函数内部使用handleResponse进行错误处理，确保错误被正确抛出
 ...
 * - reqs API 可能不需要认证，但为了兼容性，仍然集成了 Clerk 认证
 */
export const clearWebsiteReqs = async (
  website_id: number,
  token: string | null = null
): Promise<Response> => {
  const headers: Record<string, string> = {
...
/**
 * 导出网站请求结果的自定义 Mutation Hook
 *
 * 此 Hook 封装了导出网站请求结果的逻辑，使用 TanStack Query 的 useMutation 来处理异步操作
 * 允许前端通过 API 调用导出指定网站的所有请求结果
 *
 * 主要功能：
 * - 向后端API发送导出网站请求结果的请求
 * - 提供完整的状态管理（加载中、错误、成功等状态）
 *
 * @returns 返回 useMutation 的结果对象，包含以下主要属性：
 *          - mutate/mutateAsync: 触发导出操作的函数，需要传入网站ID
 *          - isLoading: 导出操作的加载状态
 *          - isError: 是否发生错误
 *          - error: 错误对象（如果有的话）
 *          - data: 导出成功的响应数据（如果有的话）
 *
 * 使用示例:
 * const { mutateAsync, isLoading, error } = useExportWebsiteReqsMutation()
 *
 ...
 *     console.log('网站请求结果导出成功')
 *   } catch (err) {
 *     console.error('网站请求结果导出失败:', err)
 *   }
 * }
 *
 * 注意事项:
 * - 此 Hook 向 /reqs/websites/{website_id}/ 端点发送PUT请求来导出网站请求结果
 * - website_id 必须是有效的数字ID，且对应网站存在于数据库中
 * - 适用于需要导出某个网站所有请求结果的场景
 * - reqs API 可能不需要认证，但为了兼容性，仍然集成了 Clerk 认证
 */
export const useExportWebsiteReqsMutation = () => {
  const queryClient = useQueryClient()
  const { getToken } = useAuth()

  return useMutation({
    mutationFn: async (website_id: number) => {
      const token = await getToken()
      return exportWebsiteReqs(website_id, token)
    },
    onSuccess: () => {
      // 导出成功后使请求列表缓存失效，确保列表显示最新状态
      queryClient.invalidateQueries({ queryKey: ['reqs'] })
    },
  })
}
...
/**
 * 清空网站请求结果的自定义 Mutation Hook
 *
 * 此 Hook 封装了清空网站请求结果的逻辑，使用 TanStack Query 的 useMutation 来处理异步操作
 * 允许前端通过 API 调用清空指定网站的所有请求结果
 *
 * 主要功能：
 * - 向后端API发送清空网站请求结果的请求
 ...
 *
 * 使用示例:
 * const { mutateAsync, isLoading, error } = useClearWebsiteReqsMutation()
 *
 * const handleClear = async (websiteId) => {
 *   try {
 *     await mutateAsync(websiteId)
 *     console.log('网站请求结果清空成功')
 *   } catch (err) {
 *     console.error('网站请求结果清空失败:', err)
 *   }
 * }
 *
 * 注意事项:
 * - 此 Hook 向 /reqs/websites/{website_id}/ 端点发送DELETE请求来清空网站请求结果
 * - 清空成功后会自动使 ['reqs'] 查询缓存失效，确保列表显示最新状态
 * - 适用于需要清空某个网站所有请求结果的场景
 * - website_id 必须是有效的数字ID，且对应网站存在于数据库中
 * - 清空操作会永久删除指定网站的所有请求结果，请谨慎使用
 * - 建议在UI中添加二次确认机制，防止误操作
 * - reqs API 可能不需要认证，但为了兼容性，仍然集成了 Clerk 认证
 */
export const useClearWebsiteReqsMutation = () => {
  const queryClient = useQueryClient()
  const { getToken } = useAuth()
  ...
  return useMutation({
    mutationFn: async (website_id: number) => {
      const token = await getToken()
      return clearWebsiteReqs(website_id, token)
    },
    onSuccess: () => {
