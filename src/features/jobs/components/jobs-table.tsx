 ...
 */
export function JobsTable({ data = [], pager = undefined, isLoading = false, isFetching = false }: DataTableProps) {
  // 表格状态管理
  // 从 context 获取搜索参数
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _, filterValue) => {
      const searchKeys = ['task_name', 'task_slug', 'uid', 'address']
  ...
  return (
    ...
    >
      ...
      {/* 数据表格工具栏 */}
      {/* 包含搜索框和过滤器，允许用户搜索任务名称/标识/节点信息，以及按任务结果状态过滤 */}
      <DataTableToolbar
        table={table} // 传递表格实例给工具栏组件
        // 不指定 searchKey 则使用全局过滤（支持跨多列 OR 搜索）
        // searchKey={["task_name", "task_slug", "node_id", "node_address"]}
        // 搜索框占位符文本
        searchPlaceholder='过滤 任务名称/标识/节点信息...'
        // 定义过滤器配置
        filters={[
          {
            columnId: 'task_status', // 对应列的ID
            title: '任务状态', // 过滤器标题
