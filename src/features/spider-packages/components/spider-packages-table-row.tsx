...
interface SpiderPackagesTableRowProps {
  row: Row<SpiderPackageItemData>
  rowIdx: number
  isSelected: boolean
  getPinningStyles: (column: Column<SpiderPackageItemData>) => PinningStyles
}

function SpiderPackagesTableRowBase({
  row,
  rowIdx,
  isSelected,
  getPinningStyles,
}: SpiderPackagesTableRowProps) {
  return (
    <TableRow
      key={row.id}
...
export const SpiderPackagesTableRow = SpiderPackagesTableRowBase
