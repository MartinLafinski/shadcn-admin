...
// 可用性标签
import { enableLabels } from "@/features/entrypoints/data/labels"
// 批量操作控件
...
interface DataTableProps {
  /**
   * 入口点数据列表
   * 类型为 EntrypointData 数组，包含入口点的基本信息
   */
  data: EntrypointData[] | undefined
  /**
   * 分页信息
 ...
 */
export function EntrypointsTable({ data = [], pager = undefined, isLoading = false, isFetching = false }: DataTableProps) {
  // 表格状态管理
  // 从 context 获取搜索参数
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _, filterValue) => {
      const searchKeys = ['entrypoint_name', 'entrypoint_slug', 'entrypoint_url']
