...
export function Search({
  ...
  useEffect(() => {
    setKeyword(searchParams?.template_keyword ?? '')
  }, [searchParams.template_keyword])

  // 当URL中的template_enabled参数发生变化时，同步更新本地enabledValue和selectedLabel状态
  // 同时根据当前的enabledValue值找到对应的标签文本进行显示
  useEffect(() => {
    const value = searchParams?.template_enabled ?? undefined
  ...
  const handleSearch = () => {
    // 更新本地状态和API参数
    setSearchParams({
      template_keyword: keyword || undefined,
      template_enabled: enabledValue,
      page: 1,
  ...
  const handleReset = () => {
    ...
    // 重置搜索参数
    const resetParams = {
      template_keyword: undefined,
      template_enabled: undefined,
      page: 1,
      size: searchParams.size,
    }
