...
import { DictionariesBulkActions } from './actions/dictionaries-bulk-actions'
import { dictionariesColumns } from './dictionaries-columns'
import {
  useDictionariesSearch,
  useDictionariesActions,
} from './dictionaries-provider'
import { DictionaryTableRow } from './dictionaries-table-row'
...
interface DataTableProps {
  data: DictionaryData[] | undefined
  pager?: PaginationInfoData
  isLoading?: boolean
  isFetching?: boolean
}

const DictionariesTableComponent = ({
  data = [],
  pager,
  isLoading = false,
  isFetching = false,
  ...
  const pinnedRightIds = useMemo(() => ['dictionary_enabled', 'actions'], [])
  ...
  function pinningStyles(column: Column<DictionaryData>) {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _, filterValue) => {
      const searchKeys = ['dictionary_name', 'dictionary_slug']
  ...
  return (
    ...
    >
      ...
      <DataTableToolbar
        table={table}
        searchPlaceholder='过滤 属性字典名称/标识...'
        filters={[
          {
            columnId: 'dictionary_enabled',
            title: '可用',
        ...
        storageKey='dictionaries-column-vis'
      />
      {viewMode === 'table' ? (
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell
                    colSpan={dictionariesColumns.length}
      ...
      <DictionariesBulkActions table={table} />
    </div>
  )
}

export const DictionariesTable = DictionariesTableComponent
